...
const ModalComponent = ({ showModal, handleCloseModal, route }) => {
  const [checkedRadioButtonId, setCheckedRadioButtonId] =
    useState('radioOption1')

  const handleCheckChange = (event) => {
    setCheckedRadioButtonId(event.target.id)
  }
  return (
    <Modal centered show={showModal} onHide={handleCloseModal} size="xl">
      <Modal.Header
        closeButton
        style={{ backgroundColor: '#6187DA', color: 'white' }}
      >
      ...
      <Modal.Body style={{ backgroundColor: '#6187DA', color: 'white' }}>
        <div className="d-flex flex-column align-items-center text-center">
          <p>
            Você deseja que o elemento identidade seja revelado para você no
            início do jogo?
          </p>
          <p>
            Lembrando que isso não vai facilitar sua jogabilidade, pois no caso
            de revelar a identidade, haverão outros empecilhos
          </p>
        </div>
      </Modal.Body>
      <Modal.Footer
      ...
      >
        <Container
          ...
          style={{ backgroundColor: '#6187DA', width: '60%' }}
        >
          ...
          >
            <ButtonLink
              route={
                route +
                '&identidade=' +
                (checkedRadioButtonId.endsWith('1') ? 'true' : 'false')
              }
              value="Iniciar"
